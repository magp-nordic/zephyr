...
int z_nrf_grtc_wakeup_prepare(uint64_t wake_time_us)
{
	...
	uint32_t minimum_latency_us =
		nrfy_grtc_waketime_get(NRF_GRTC) + nrfy_grtc_timeout_get(NRF_GRTC) + 1000;
	uint32_t chan;
	int ret;
	...
	/* Make sure that it is not too close to wake up event trigger. */
	else if (now + minimum_latency_us > get_comparator(systemoff_channel)) {
		k_spin_unlock(&lock, key);
		return -EINVAL;
	}
